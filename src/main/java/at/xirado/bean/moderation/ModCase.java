...
public class ModCase
{
    ...
    private ModCase(UUID uuid, CaseType caseType, long guildId,
                    ...
                    long createdAt, long duration, boolean active)
    {
        this.uuid = uuid;
    ...
    {
        this(uuid, caseType, guildId, targetId, moderatorId, reason, createdAt, -1, active);
    }
    ...
    public static ModCase createModCase(CaseType type, long guildId, long targetId, long moderatorId, long duration, String reason)
    {
        try (Connection connection = Database.getConnectionFromPool();
            PreparedStatement ps = connection.prepareStatement("INSERT INTO modCases (uuid, caseType, guild, user, moderator, reason, createdAt, duration, active) values (?,?,?,?,?,?,?,?,1)"))
        {
            ...
            ps.execute();
            return new ModCase(uuid, type, guildId, targetId, moderatorId, reason, creationTime, duration, true);
        }
        catch (Exception ex)
        {
    ...
    public static ModCase retrieveModCase(UUID uuid)
    {
             ...
             PreparedStatement ps = connection.prepareStatement("SELECT * FROM modCases WHERE uuid = ?"))
        {
            ...
            return new ModCase(uuid, CaseType.fromId(rs.getByte("caseType")), rs.getLong("guild"), rs.getLong("user"), rs.getLong("moderator"), rs.getString("reason"), rs.getLong("createdAt"), rs.getLong("duration"), rs.getBoolean("active"));
        }
        catch (Exception ex)
        {
